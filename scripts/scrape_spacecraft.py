#/usr/bin/env python
"""accesses satellite data from nasa CDA and creates <sat>.txt for SWMF input
"""
import os,sys
...
def write_SWMF_satfile(df,outname,outpath,**kwargs):
    ...
    with open(os.path.join(outpath,outname),'w') as f:
        ...
        print(f'\t\003[92m Created\003[00m {os.path.join(outpath,outname)}')
...
def main() -> None:
    ...
    if PLOT_DATA:
        ...
        for sat in cluster_pos.keys():
            equitorial.scatter(cluster_pos[sat]['x_gsm'],
        ...
        for sat in themis_pos.keys():
            equitorial.scatter(themis_pos[sat]['x_gsm'],
